...
df=pd.read_csv('/home/seema/plot1.csv', sep=',',header=None)
df.values
...
# plot
fig = plt.figure()
plt.plot(x,df[1], '--', label='exhaust fan in')
plt.plot(x,df[2], ':', label='exhaust fan out')
plt.plot(x,df[3], '-.', label='fresh air in')
plt.plot(x,df[4], '-', label='fresh ait out')
plt.gcf().autofmt_xdate()
fig.suptitle('SUPPLY AND EXHAUST FAN FAULT BEFORE PREPROCESSING', fontsize=20)
plt.xlabel('TIME', fontsize=18)
plt.ylabel('EXHAUST_FAN_IN RELATIVE HUMIDITY', fontsize=16)
plt.legend(loc='upper right')
fig.savefig('before_preprocessing_SUPPLY_AND_EXHAUST_FAN_FAULT_BEFORE_PREPROCESSING_all.jpg')
#ax1 = plt.subplot(311)
